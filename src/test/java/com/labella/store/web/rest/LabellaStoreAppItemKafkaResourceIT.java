...
import com.labella.store.config.KafkaProperties;
...
class LabellaStoreAppItemKafkaResourceIT {
    ...
    void setup() {
        ...
        LabellaStoreAppItemKafkaResource kafkaResource = new LabellaStoreAppItemKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/labella-store-app-item-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/labella-store-app-item-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
